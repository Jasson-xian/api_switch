...
<script>
	...
	export default {
		...
		computed: {
			xaxis: function() {
				var x_temp = []
				for(var i = 0; i < this.recordDataMon.length; i++) {
					x_temp[i] = this.recordDataMon[i].x
				}
				return x_temp
			},
			yaxis: function() {
				...
				for(var i = 0; i < this.recordDataMon.length; i++) {
					y_temp.port[i] = Number(this.recordDataMon[i].port)
					y_temp.sum[i] = Number(this.recordDataMon[i].sum)
					y_temp.icmp[i] = Number(this.recordDataMon[i].icmp)
				}
				return y_temp
			}
		},
		...
		methods: {
			...
			loadMonLine() {				
			  ...
			  this.picchart = Highcharts.chart('monRecodeline', {
                    chart:{
                    	zoomType:'x'
                    },
					title: {
