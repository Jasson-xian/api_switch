<template>
    <div class="wrap col-xs-6 col-md-6 col-lg-6">
        <div class="bgcolor">
            ...
            <div class="head">
                <h3>内存使用率</h3><span>单位：%</span></div>
            <transition name='fade'>
            <div id="memorypic" v-show='show'>
            </div>
            </transition>
        </div>
    </div>
         ...
         picData:function(){
            if(this.num==0){
                ...
                this.intervalid = setInterval(() => {
                    let chart =$("#memorypic").highcharts()
                     let x = this.picData.pop().time*1000?this.picData.pop().time*1000:0
                    let y = this.picData.pop().memory*100?this.picData.pop().memory*100:0
        ...
        loadSpline(picData) {
            ...
            Highcharts.chart('memorypic',{
                ...
                tooltip: {
                       formatter: function () {
                           return '<b></b>' +
                               Highcharts.dateFormat('%H:%M:%S', this.x) + '<br/>内存：' +
                        ...
                        var data = [];
                            for(let j=0;j<picData.length;j++){
                                let obj={
                                    ...
                                    y:picData[j].memory*100?picData[j].memory*100:0
                                }
                                data.push(obj);
                            }
                        return data;
