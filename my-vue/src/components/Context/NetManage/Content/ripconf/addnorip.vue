<template>
	<div @keyup.enter="setnorip()" class="modal" id="addnorip" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-content">
				<div class="modal-body">
					...
					<table class="table">
						<thead>
							<tr>
								<th colspan="2"> {{oper =='edit'?'修改':'添加'}}真实IP保留网段配置
						...
						<tbody v-show="result!='ok' && result!='loading'">
							...
							<tr>
								...
								<td>
									...
									<!--<ipinput class="nomask" :empty=false></ipinput>-->
							...
							<tr>
								<td>线路：</td>
								<td>
									<select  class="noroute" :class="[options.length<=0?'nosele':'']" data-container="body" data-toggle="popover" @focus="noEmpty('.noroute')"
				...
				<div class="modal-footer">
					...
					<button class="enter button button--aylen" @click="setnorip()">确认</button>
		...
		methods: {
			ipVal(ele){
			 ...
			 setnorip(){
				...
				if( this.testMacIp('.noabcd','ip') && this.masktest()
				 && this.testVal('.novlanid', 'vlanid') && this.testVal('.noroute', 'allpass')){
					var noripconfig = {
						type:this.oper == 'edit'?"edit_norip":"add_norip",
						abcd:noabcd,
						mask:nomask,
						vlanid:$(".novlanid").val(),
						groupid:$(".noroute").val()
					...
					if(this.oper == 'edit'){
						noripconfig.hash_code = $('.hash_code_no').html()
					}	
					this.$emit('getAddModData',noripconfig);
				}				
            },
			testMacIp(ele,type){				
				if(type=='ip'){
