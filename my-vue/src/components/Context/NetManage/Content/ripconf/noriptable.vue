<template>
    <div class="table-wrap norip-table" style="min-height: 300px;position: relative;">
        ...
        <table>
            <thead>
                <tr>
                    <th style="width: 1200px;" colspan="4">真实IP保留网段配置</th>
                    <th colspan="2">
                        <ul class="butn">
                            <li @click='addData()'  @mouseenter="actived('add')" @mouseleave="unactived()"  data-toggle="modal" data-target="#addnorip">
                                <img v-if="type=='add'" src="../../../../img/b_add.png" />
                                <img v-else src="../../../../img/add.png" />
                                <span :class="[type=='add'?'actived':'unactived']">添加</span>
                            </li>
                            <li @mouseenter="actived('delete')" @mouseleave="unactived()" v-on:click='rmnoripMsg()' class="deletenotag" data-placement="bottom">
                            ...
                            <addnorip v-on:getAddModData="getAddModData" :oper ="oper" :result = 'result' :errorMsg ='errorMsg'></addnorip>
                            <botmakesure v-on:rm="rmNorip()" :result = 'result' :name ='name' :errorMsg ='errorMsg'></botmakesure>
                        </ul>
                    </th>
                </tr>
                <tr>
            ...
            <transition name="fade"  mode="out-in">
                <tbody v-if='show'>
                    <tr class="change" v-for="(norip, index) in data"  @dblclick.stop = "modData(index)">
                        <template v-if="norip.hash_code">
                            <td style="width: 40px;">
                            ...
                            <td style="display:none;">{{ norip.hash_code }}</td>
                            <td style="width: 275px;"><label :for="index +'no'">{{ norip.ABCD }}</label></td>
                            <td style="width: 275px;"><label :for="index +'no'">{{ norip.MASK }}</label></td>
                            <td style="width: 275px;"><label :for="index +'no'">{{ norip.vlanid }}</label></td>
                            <td style="width: 275px;"><label :for="index +'no'">线路{{ norip.groupid }}</label></td>
                        </template>
                        <template v-else>
...
<script>
    ...
    import addnorip from '../ripconf/addnorip.vue'
	...
	export default {
        ...
        methods: {
        	reset(){
                ...
                $('#addnorip .noabcd input').val('').css('border', 'none')
                $('#addnorip .nomask input').val('').css('border', 'none')
			...
			getAddModData(noripconfig){
				this.result = "loading"
				netservice.setAllnet(noripconfig)
				.then((recvdata) => {
