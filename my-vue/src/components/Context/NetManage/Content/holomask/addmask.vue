<template>
	<div  class="modal" id="addmask" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<table class="table">
						...
						<tbody v-show="result=='add'">
							<tr>
								<td>策略名：</td>
								<td>
									<input class="name" v-model="name" type="text" @blur="testVal('.name', 'CLname')" @focus="noEmpty('.name')" @keyup.enter="setMask()" placeholder="我是策略名">
							...
							<tr>
								<td>源IP：</td>
								<td>
									<!--<input type="text" class="rip" @blur="testVal('.rip', 'ip')" @focus="noEmpty('.rip')" @keyup.enter="setMask()" placeholder="192.168.2.1"/>-->
									<ipinput class="rip" :empty=false></ipinput>
							...
							<tr>
								...
								<td colspan="3" style="position: relative;">
									<textarea class="vip" v-model="vip" placeholder='192.168.2.1
192.168.2.2...'	@blur="chkvips()" @focus="noEmpty('.vip')"></textarea>
									<span class="mi" style="position: absolute; top:15px;">* 配置多个请换行输入，<br/>&nbsp;&nbsp;一行一个ip地址，<br/>&nbsp;&nbsp;最多可输10个。</span>
								</td>
							</tr>
							<tr>
								<td>VLAN ID：</td>
							...
							<tr>
								<td>物理线路：</td>
								<td colspan="3">
									<select class="road"  @focus="noEmpty('.road')">
	...
	export default {
		...
		methods: {
			...
			setMask(){
				this.errorMsg = ""
				$('.notice').hide()
                var nameTest= this.testVal('.name', 'CLname')
				var ripTest = this.testMacIp('.rip', 'ip')
				var vipTset = this.chkvips()
				if(nameTest&&ripTest&&vipTset){
					var vipArr = this.vip.split("\n")
					var vip = new Array()
