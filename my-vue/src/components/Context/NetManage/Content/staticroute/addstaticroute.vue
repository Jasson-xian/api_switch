<template>
	<div @keyup.enter="setRoute()" @click.self ='dataFresh()' class="modal" id="addstaticroute" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-content">
				<div class="modal-body">
					<table class="table">
						<thead>
							<tr>
								<th colspan="2">添加静态路由配置
									<span class="close" data-dismiss="modal"  @click ='dataFresh()' aria-hidden="true">&times;</span>
								</th>
							</tr>
						</thead>
						<!--正在请求-->
						<tbody class="result" v-show="result=='loading'">
							<div style="height: 172px;padding-top: 20px;">
						...
						<tbody class="result" v-show="result=='success'">
							<div style="height: 172px;padding-top: 70px;">
						...
						<tbody v-show="result=='add'">
							<tr>
								<td>目的网段：</td>
								<td style="display: flex;align-items: center; width: 360px;margin-top: 8px;">
									<!--<input class="input abcd" v-model="abcd" type="text"  @focus="noEmpty('.abcd')"
									@blur="testVal('.abcd', 'ipRang')" placeholder="192.168.2.0/24">-->
									<staticinput class="abcd" :empty=false></staticinput>
							...
							<tr>
								<td>网关地址：</td>
								<td>
									<!--<input class="input gateway" v-model="gateway" type="text"  @focus="noEmpty('.gateway')"
									@blur="testVal('.gateway', 'ip')"  placeholder="192.168.2.0">-->
									<ipinput class="gateway" :empty=false></ipinput>
									<span class="mi">* 必填</span>									
								</td>
							</tr>	
							<tr>
	...
	export default {
		...
		methods: {
			setRoute(){
				this.errorMsg = ""
				$('.notice').hide()
				if( this.testipVal('.abcd', 'ipRang') && this.testMacIp('.gateway', 'ip') ){
					let conf = {
						oper:"add",
						abcd:this.mgipVal('.abcd'),
						gateway:this.ipVal('.gateway')
					};
					this.result = "loading"
