<template>
    <div @keyup.enter="setvip()" class="modal" id="addvip" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-content">
				<div class="modal-body">
					...
					<table class="table">
						<thead>
							<tr>
								<th colspan="2"> {{oper =='edit'?'修改':'添加'}}虚拟IP网段配置
				...
				<div class="modal-footer">
					...
					<button class="enter button button--aylen" @click="setvip()">确认</button>	
			 ...
			 setvip(){
				...
				if( this.testMacIp('.abcd','ip') && this.masktest()
				 && this.testVal('.vlanid', 'vlanid') && this.testVal('.route', 'allpass') ){
					var vipconfig = {
						type:this.oper == 'edit'?"edit_vip":"add_vip",
					    abcd:abcd,
						mask:mask,
						vlanid:$(".vlanid").val(),
						groupid:$(".route").val()
					...
					if(this.oper == 'edit'){
						vipconfig.hash_code = $('.hash_code').html()
					}
					this.$emit('getAddModData',vipconfig);
				}
            },
			getGroups(){
