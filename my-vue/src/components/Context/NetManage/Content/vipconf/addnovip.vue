<template>
	<div @keyup.enter="setnovip()"  class="modal" id="addnovip" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-content">				
				<div class="modal-body">
					...
					<table class="table">
						<thead>
							<tr>
								<th colspan="2"> {{oper =='edit'?'修改':'添加'}}虚拟IP保留网段配置
				...
				<div class="modal-footer">
					...
					<button class="enter button button--aylen" @click="setnovip()">确认</button>
			 ...
			 setnovip(){
				...
				if( this.testMacIp('.noabcd','ip') && this.masktest()
				 && this.testVal('.novlanid', 'vlanid') && this.testVal('.noroute', 'allpass')){
					var novipconfig = {
						type:this.oper == 'edit'?"edit_novip":"add_novip",
						abcd:noabcd,
						mask:nomask,
						vlanid:$(".novlanid").val(),
						groupid:$(".noroute").val()
					...
					if(this.oper == 'edit'){
						novipconfig.hash_code = $('.hash_code_no').html()
					}
					this.$emit('getAddModData',novipconfig);
				}
			 },
			 getGroups(){
