<template>
	<div @keyup.enter="setAddr()" class="modal" id="altstaticaddr" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<table class="table">
						<thead>
							<tr>
								...
								<th class="id" style="display: none;"></th>
							</tr>
						</thead>
						<!--正在请求-->
						...
						<tbody v-show="result=='add'">
							...
							<tr>
								<td>Mac地址：</td>
								<td>
									<!--<input class="addr-input altmac" type="text" @focus="noEmpty('.altmac')"  @blur="testVal('.altmac', 'mac')" placeholder="如：1a:2b:3c:4d:5e:6f">-->
									<macinput class="altmac" :empty=false></macinput>
							...
							<tr>
								<td>IP地址：</td>
								<td>
									<!--<input class="addr-input altabcd" type="text" @focus="noEmpty('.altabcd')" @blur="testVal('.altabcd', 'ip')" placeholder="如：192.168.2.1">-->
									<ipinput class="altabcd" :empty=false></ipinput>
									<span class="mi">* 必填</span>
								</td>
							</tr>   
						</tbody>
	...
	export default {
		...
		methods: {
			...
			setAddr(){
				this.errorMsg = ""
				$('.altnotice').hide()
				if(this.testMacIp('.altmac','mac') && this.testMacIp('.altabcd','ip') ){
					let conf = {
						oper:"edit",
						abcd:this.ipVal('.altabcd'),
						mac:this.macVal('.altmac'),
						id:$('.id').html()
					};
					this.result = "loading"
					...
					.then((recvdata) => {
						...
						if(recvdata.success && recvdata.data.result == "ok") {
							...
							$('.altnotice').show().addClass('animated shake')
							setTimeout(function() {
								$('.altnotice').removeClass('animated shake')
							}, 200)
						}
					})
				}
