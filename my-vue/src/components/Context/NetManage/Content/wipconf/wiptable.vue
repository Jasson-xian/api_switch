<template>
    <div class="table-wrap" style="min-height: 300px;position: relative;">
        ...
        <table>
            <thead>
                <tr>
                    <th style="width: 1200px;" colspan="5">外网IP网段配置</th>
                    <th colspan="2">
                        <ul class="butn">                         
                            <li @click='addData()' @mouseenter="actived('add')" @mouseleave="unactived()" data-toggle="modal" data-target="#addwip">
                                <img v-if="type=='add'" src="../../../../img/b_add.png" />
                                <img v-else src="../../../../img/add.png" />
                                <span :class="[type=='add'?'actived':'unactived']">添加</span>
                            </li>
                            <li @mouseenter="actived('delete')" @mouseleave="unactived()" v-on:click='rmwipMsg()' class="deletetag" data-placement="bottom">
                            ...
                            <addwip v-on:getAddModData="getAddModData" :oper ='oper' :result = 'result' :errorMsg ='errorMsg'></addwip>
                            <topmakesure v-on:rm="rmWip()" :result = 'result' :name ='name' :errorMsg ='errorMsg'></topmakesure>
                        </ul>
                    </th>
                </tr> 
                <tr>
            ...
            <transition name="fade" mode="out-in">
                <tbody v-if = 'show'>
                    <tr class="change" v-for="(wip, index) in data"  @dblclick.stop = "modData(index)">
                        <template v-if="wip.hash_code">
                            <td style="width: 40px;">
                            ...
                            <td style="display:none;">{{ wip.hash_code }}</td>
                            <td style="width: 220px;"><label :for="index">{{ wip.ABCD }}</label></td>
                            <td style="width: 220px;"><label :for="index">{{ wip.MASK }}</label></td>
                            <td style="width: 220px;"><label :for="index">{{ wip.gateway }}</label></td>
                            <td style="width: 220px;"><label :for="index">{{ wip.vlanid }}</label></td>
                            <td style="width: 220px;"><label :for="index">线路{{ wip.groupid }}</label></td>
                        </template>   
                        <template v-else>
...
<script>
    ...
    import addwip from '../wipconf/addwip.vue'
        ...
        methods: {
			...
			modShow(which) {
                ...
                setTimeout(() => {				
                    ...
                    this.arrip('.gateway',moddata.gateway)
