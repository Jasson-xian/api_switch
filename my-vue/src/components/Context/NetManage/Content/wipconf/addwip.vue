<template>
    <div @keyup.enter="setwip()"  class="modal" id="addwip" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-content">
				<div class="modal-body">
					...
					<table class="table">
						<thead>
							<tr>
								<th colspan="2">{{oper =='edit'?'修改':'添加'}}外网IP网段配置
						...
						<tbody v-show="result!='ok' && result!='loading'">
							...
							<tr>
								<td>网关地址：</td>
								<td>
									<!--<input  type="text" @focus="noEmpty('.gateway')"
									@blur="testVal('.gateway', 'ip')" class="gateway" placeholder="192.168.1.1">-->
				...
				<div class="modal-footer">
					...
					<button class="enter button button--aylen" @click="setwip()">确认</button>
		...
		methods: {
			...
			masktest(){
			 ...
			 setwip(){
				...
				let gateway=this.ipVal('.gateway')
				if( this.testMacIp('.abcd','ip') && this.masktest() &&this.testMacIp('.gateway','ip')
				&& this.testVal('.vlanid', 'vlanid') && this.testVal('.route', 'allpass') ){				
					var wipconfig = {
						type:this.oper == 'edit'?"edit_wip":"add_wip",
						abcd:abcd,
						mask:mask,
						vlanid:$(".vlanid").val(),
						groupid:$(".route").val(),
					...
					if(this.oper == 'edit'){
						wipconfig.hash_code = $('.hash_code').html()
					}
